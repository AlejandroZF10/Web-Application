...
public class NuevoExamen extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        out.println("<form method='post' id='f1' action='CrearExamen'>");
        out.println("<button form='f1' type='submit' class='button' >");
        out.println("Crear Examen");
        out.println("</button>");
        out.println("<br>");
        out.println("ID del Examen: <input type='text' name='id' class='caja' />");
        out.println("<br>");
        out.println("<br>");

        String carpeta = request.getSession().getServletContext().getRealPath("/"); //ruta donde se guardan los archivos xml
        ...
        File[] archivo = ruta.listFiles(); //arreglo que contiene los archivos de la ruta 
        //creamos tabla que recopilara todos los examenes 
            ...
            for (int i = 0; i < archivo.length; i++) {
                ...
                if (archivo[i].isDirectory() == false) {
                    if (archivo[i].getName().startsWith("pregunta") && archivo[i].getName().startsWith("preguntaM")) { //verificamos que el nombre del archivo empiece con la palabra 'preguntaM'
                        ...
                        //el checkbos nos ayuda a saber que preguntas seran oncluidas en el examen
                        out.println("<td><input type='checkbox' name='check"+cont+"' value='M"+archivo[i].getName().charAt(9)+"' form='f1' formaction='CrearExamen'/></td>");
                        cont++;
                    } else {
                        if (archivo[i].getName().startsWith("pregunta") && archivo[i].getName().startsWith("preguntaH")) { //verificamos que el nombre del archivo empiece con la palabra 'preguntaH'
                            ...
                            out.println("<td><input type='checkbox' name='check"+cont+"' value='H"+archivo[i].getName().charAt(9)+"' form='f1' formaction='CrearExamen'/></td>");
                            cont++;
                        }
                    }
                }
